...
| `GET /donation-campaigns/:id/progress` | público                    | Progreso de recaudo: suma de donaciones `CONFIRMED`, número de donaciones y % de la meta (sin datos del donante)                          |
...
| `GET /donations/:id/receipt/pdf`       | `donation.read`            | Descarga el PDF del recibo (copia archivada; si no hay storage, se genera al vuelo)                                                       |
| `POST /donations/:id/receipt/resend`   | `donation.create`          | Reenvía el recibo en PDF al correo del donante y lo archiva si faltaba                                                                    |
| `PATCH /donations/:id/donor-document`  | `donation.create`          | Agrega o corrige el documento del donante (`donorIdType` + `donorIdNumber`). Solo en `PENDING`: una vez confirmada, el recibo ya lo copió |
| `POST /donations/:id/confirm`          | `donation.create`          | Confirma una donación `PENDING` (ej. pago manual verificado) emite el recibo y registra el ingreso en el ledger                           |
| `POST /donations/:id/cancel`           | `donation.create`          | Cancela una donación `PENDING`                                                                                                            |
| `POST /donations/:id/refund`           | `donation.refund`          | Reembolsa una donación `CONFIRMED` y registra la reversa en el ledger                                                                     |
...
- **Toda donación confirmada entra al ledger** — `confirm` (manual o por webhook) crea en la misma transacción un asiento `INCOME` en la categoría `Donaciones` (`relatedDonationId`, `relatedCampaignId`), así los reportes de transparencia —que solo suman `financial_transaction`— nunca omiten una donación. `refund` registra el asiento `EXPENSE` compensatorio vía `reversalOfTransactionId` (el ledger es append-only). Si la categoría no existe (seed no corrido), se crea al vuelo.
- **Recibo en PDF y correo al donante** — tras el `COMMIT` de `confirm`, `DonationReceiptsService` genera el PDF (pdfkit), lo sube a Firebase Storage **privado** (`receipts/<año>/<número>.pdf`, porque lleva datos del donante), lo enlaza en `donation_receipt.pdf_media_asset_id` y lo envía adjunto a `donorEmail`. Si falla el SMTP o no hay storage, la confirmación **no** se revierte: se registra el error y el admin puede reintentar con `POST /donations/:id/receipt/resend`.
- **Recibo con validez tributaria** — `POST /donations` acepta `donorIdType` (`CC`, `CE`, `NIT`, `PASSPORT`, `PPT`) y `donorIdNumber` (se aceptan puntos/espacios, se normaliza y se valida según el tipo). Van juntos o ninguno (validado en el DTO y por el CHECK `donation_donor_id_complete` en Postgres). Al confirmar, el documento se copia a `donation_receipt.tax_id_snapshot`. El emisor del recibo se configura con `RECEIPT_ISSUER_NAME`, `RECEIPT_ISSUER_TAX_ID` (NIT de la Junta) y `RECEIPT_ISSUER_ADDRESS`.
